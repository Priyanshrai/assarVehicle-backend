...
import StateSchemaModel from '../model/state.model.js';

export var save=async (req,res,next)=>{
  var userDetails=req.body;
  var userList = await StateSchemaModel.find();
  var l=userList.length;
  var _id=l==0?1:userList[l-1]._id+1;
  userDetails={...userDetails,"_id":_id,"status":0,"info":Date()};
  var user = await StateSchemaModel.create(userDetails);
  if(user)
...
export var fetch=async (req,res,next)=>{
  ...
  var userList = await StateSchemaModel.find(condition_obj);
...
export var deleteUser=async(req,res,next)=>{
  ...
  var user = await StateSchemaModel.find({_id: id});
  if(user.length!=0){
    let result = await StateSchemaModel.deleteMany({_id:id}); 
    if(result)
...
export var updateUser=async(req,res,next)=>{
  let userDetails = await StateSchemaModel.findOne((req.body.condition_obj));
  //console.log(userDetails);
  if(userDetails){
     let user=await StateSchemaModel.updateOne((req.body.condition_obj),{$set:(req.body.content_obj)});   
     if(user)
      return res.status(201).json({"msg":"success"});
     else
      return res.status(500).json({error: "Server Error"});
...
export var login=async (req,res,next)=>{
  ...
  var userList = await StateSchemaModel.find(userDetails);
  if(userList.length!=0)
  {
